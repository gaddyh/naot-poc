...
def build_audited_ingest_image_graph(scanner, auditor, recovery_scanner=None):
    ...
    graph = StateGraph(IngestImageState)
    graph.add_node("parallel_scan_audit", nodes.parallel_scan_audit)
    graph.add_node("reconcile", nodes.reconcile)
    graph.add_node("targeted_recovery", nodes.targeted_recovery)
    ...
    graph.add_conditional_edges(
        "reconcile",
        lambda state: "targeted_recovery" if state.get("missing_regions") else "merge",
